...
main = do args <- getArgs
          if (length(args) /= 4 && length(args) /= 0)
            ...
            then playIO (InWindow "Gomoku" (840, 700) (10, 10)) (greyN 0.4) 10 (initWorld 6 3 [] Black White "normal" False) drawWorld handleInput updateWorld
          else do
            let size = read (args!!0) :: Int
            let target = read (args!!1) :: Int
            let player = read (args!!2) :: Col
            let game_type = (args!!3)
            playIO (InWindow "Gomoku" (840, 700) (10, 10)) (greyN 0.4) 10 (initWorld size target [] Black player game_type True) drawWorld handleInput updateWorld
